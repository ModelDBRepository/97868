: $Id: vecst.mod,v 1.400 2007/09/14 12:32:06 billl Exp $
...
NEURON {
  ...
  GLOBAL BVBASE, RES, VECST_INSTALLED, DEBUG_VECST
}
...
int list_vector_px4(Object*,int,double**,unsigned int);
...
static double slct (void* vv) {
  int i, j, k, m, n, p, ni, nk, na, nv[VRRY], num, fl, lc, field[VRRY], lt, rt;
  Object* lob;
  ...
  for (i=0,j=0;i<num;i++,j++) {     // pick up vectors
    ...
    if (key[i]>=EQV && key[i]<=EQX) { // EQV-EQX take extra vec arg of any size
      i++;
...
static double ird(void* vv) {
  ...
  f = hoc_obj_file_arg(1);
  nx = vector_instance_px((IvocVect*)vv, &x);
  int r=fread(&n,sizeof(int),1,f);  // size
  if (n>scrsz) { 
    if (scrsz>0) { free(scr); scr=(unsigned int *)NULL; }
    scrsz=n+10000;
    scr=(unsigned int *)ecalloc(scrsz, sizeof(int));
...
static double fread2(void* vv) {
  ...
  if (type==6 || type==16) {         // unsigned ints
    ...
    xs=(unsigned int*)scr;
    int r=fread(xs,sizeof(int),n,fp);
    ...
    int r=fread(xf,sizeof(float),n,fp);
...
static double nearall (void* vv) {
  ...
  Object *ob;
  void* vvl[4];
  double *x, *y, *vvo[4], targ, dist, new_, max, tmp;
  nx = vector_instance_px((IvocVect*)vv, &x);
  max = *getarg(1);
...
static double samp (void* vv) {
  ...
  nx = vector_instance_px((IvocVect*)vv, &x); // dest
...
static double onoff(void* vv) {
  ...
  n = vector_instance_px((IvocVect*)vv, &st);
...
static double bpeval(void* vv) {
  ...
  double add,div;
  double *vo, *outp, *del;
  n = vector_instance_px((IvocVect*)vv, &vo);
...
static double xing (void* vv) {
  ...
  tvf=0;
  ndest = vector_instance_px((IvocVect*)vv, &dest);
  nsrc = vector_arg_px(1, &src);
  if (ifarg(3)) {
...
static double uniq (void* vv) {
  ...
  if (nz) vector_resize((IvocVect*)voi[1], cnt);
  return (double)cnt;
}
  ...
  if (flag) { // selected only -- need to write
    ...
    for (j=0; j<cols; j++) {
      if (fcd[(int)col[j]]==0) {  
        ...
        hoc_pushobj(&obo);
      } else { printf("nqvt ERRC unhandled type: %f\n",fcd[j]); hxe(); }
    }
    hoc_pushx((double)i);
...
static double vrdh (void* vv) {	
  ...
  for (i=0;i<num;i++) { 
    int r=fread(&n,sizeof(int),2,f); // n[1] is type
...
PROCEDURE install_vecst () {
  if (VECST_INSTALLED==1) {
    printf("$Id: vecst.mod,v 1.400 2007/09/14 12:32:06 billl Exp $\n")
  } else {
  VECST_INSTALLED=1
  VERBATIM {
  int i,j;
